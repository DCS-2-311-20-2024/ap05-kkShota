...
function init() {
  ...
  // テクスチャの読み込み
  const textureLoader = new THREE.TextureLoader(); //空欄
  const moonTexture = textureLoader.load("moon.jpg"); //空欄
  const testTexture = textureLoader.load("moonTester.jpg");//空欄
  // 正20面体の作成
  const geometry = new THREE.IcosahedronGeometry();
  const material = new THREE.MeshLambertMaterial();
  material.map = testTexture; //空欄
  // UVマッピング
